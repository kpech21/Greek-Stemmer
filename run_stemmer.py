...
def run():
    ...
    while True:
        ...
        word = unicode(raw_input('Please give a word: ').strip(), 'utf-8')      # convert the given word to unicode
        ...
        if re.search('[a-zA-Z]', word):  # case of non-Greek word
            print 'Non-Greek word'
            continue
        else:
            if pos in load_data.get_pos['Verb']:  # if given word is verb or modal verb
